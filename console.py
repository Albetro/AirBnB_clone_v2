...
def parse(arg):
    ...
    if curly_braces is None:
        if sq_brackets is None:
            ...
            brackets = split(arg[:sq_brackets.span()[0]])
            brackets1 = [token.strip(",") for token in brackets]
            brackets1.append(sq_brackets.group())
            return brackets1
    else:
        brackets = split(arg[:curly_braces.span()[0]])
        brackets1 = [token.strip(",") for token in brackets]
        brackets1.append(curly_braces.group())
        return brackets1
    return []
